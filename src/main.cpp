...
#define TESTENC false // если поставить true то тогда в режиме монитора порта будет выводить импульсы енкодера
// в рабочем варианты надо обязательно поставить false

#define DEBUG false // выставить true если надо посмотреть импульсы от концевиков и кнопок, поставить false в рабочей программе

#define TESTIMP false
...
#define U_key A0 // Кнопка вверх  А0
#define D_key A1 // Кнопка вниз   А1

#define U_konc A2 // Концевик вверху А2
#define D_konc A3 // Концевик внизу А3

#define Laser A4 // Лазерный уровнемер A4
...
#define pps 17 //   количество сигналов от энкодера для одного шага двигателя !!!меньше 5 не надо ставить - иначе не успеваетработать

#define pps_pol 17 //  регулировка скорости движка в режиме половинной скорости. Количество сигналов от энкодера для одного шага двигателя на режиме половинной скорости, если поставить 36 (2*pps), то скорость будет при старте в два раза медленнее, если 54 (3*pps) то в три раза медленнее
// 0 - отключить половинную скорость

//****** HOLD **************

#define LR_hold_time 1700 // количество миллисекунд при удержании клавиш право лево при которых они фиксируются и удерживаются до нажатия противоположной клавиши или до сработки любого концевика

/**** CODEDAT *********/
#define pps_start 6 //  нельзя ставить = 0!!! количество сигналов от энкодера после которых включается режим половинной скорости (триггер старта)

#define time_lotok 1100         // через сколько миллисекунд после старта режима половинной скорости срабоает датчик лотка
#define time_paper 1110         // через сколько миллисекунд после старта режима половинной скорости срабоает датчик бумаги
#define time_lotok_reset 600000 // через сколько миллисекунд после старта режима половинной скорости сбросится датчик лотка

#define time_half 2300 // через сколько миллисекунд после старта режима половинной скорости движек начнет крутить с обычной скоростью
// bool TRIG_CONTROL_CODEDAT = false;

/********* ftnl *************/

#define time_UpD 320 // через сколько миллисекунд после пропадания сигнала наезда на препятствие отключится вертикальное перемещение должно быть меньше 3000!!!
//***************
